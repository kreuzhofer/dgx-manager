...
- [x] **Heterogeneous cluster guard** — `checkRecipeArchAdmission` (`admission/recipe-arch.ts`) is applied **per cluster member** in `routes/deployments.ts`, so a mixed-arch cluster is refused with a 400 naming the node. It was wired to `agentHub.getRecipes()` alone — the *sparkrun* catalog — so every `@dgxrun/` deploy (i.e. every multi-node deploy) missed the lookup and skipped the check entirely ([#44](https://github.com/kreuzhofer/dgx-manager/issues/44), 2026-08-30). Fixed 2026-09-04: the lookup falls back to `getDgxrunCatalog()`, covered by three integration tests including the mixed-arch cluster. Inline `recipeYaml` / `recipePath` stay deliberately unguarded as the remote-recipe-development escape hatch.
