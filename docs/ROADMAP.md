...
### Pending hardware verification (2026-07-10 fixes) — [#42](https://github.com/kreuzhofer/dgx-manager/issues/42)
...
- [ ] **Remove dead offline FP8 quantize plumbing** — filed as [#43](https://github.com/kreuzhofer/dgx-manager/issues/43) (2026-08-30), `ready-for-human`: the deletion is mechanical, the go/no-go is a direction call. The ticket adds one check this note lacks — confirm no `FineTuneJob` row carries a non-null `quantizedPath` before dropping the columns, since that column is the only record of where a real on-disk artifact lives. Now that FP8 deploys use vLLM's on-load `--quantization fp8`. Specifically: `POST /api/finetune/:id/quantize`, the agent `cmd:finetune:quantize` handler, `quantizationStatus`/`quantizedPath`/`quantizationLog`/`quantizedAt` columns, the dashboard Quantize button, `scripts/quantize_fp8.py` in the recipes repo. Driven by an unresolvable llmcompressor↔transformers pin conflict (llmcompressor 0.10 pins `transformers<=4.57.6`; qwen3_5 needs `>=5.0`). Left in place for now so the change footprint stays small; can be ripped out once we're sure on-load FP8 is the durable answer.
...
- [x] **VRAM admission guard ✅** — shipped as `checkVllmVramAdmission` (`server/src/admission/vram.ts`): a pure `computeVramShortfall` under a Prisma-coupled orchestrator, called on solo + cluster deploys (`routes/deployments.ts:469`), restart (`:911`) and fine-tune deploy (`routes/finetune.ts:1141`). Returns 409 with the conflicting deployments rather than auto-evicting. The original wording named a `recipe.vram_required` field that **has never existed** in any recipe or type — admission works off `gpu_memory_utilization` × reported `vramTotal` minus a 5% safety margin instead.
  - [ ] **Residual, newly relevant on discrete-VRAM nodes:** admission checks *fraction of total vs currently free*, not *do these weights fit at all*. A recipe whose checkpoint exceeds the card is admitted and then fails minutes into the load — e.g. Qwen3.8-27B at BF16 is 51.7 GiB against aihost01's 31.8 GiB usable. A cheap pre-dispatch guard comparing declared/derived weight size against `node.vramTotal` would catch it; it did not matter while every node had 128 GB of unified memory.
- [ ] **Heterogeneous cluster guard** — *partly built already; this wording was misleading.* `checkRecipeArchAdmission` (`admission/recipe-arch.ts`) exists and is correctly applied **per cluster member** (`routes/deployments.ts:446-465`), so a mixed-arch cluster is refused with a 400 naming the node. The real gap, filed as [#44](https://github.com/kreuzhofer/dgx-manager/issues/44) (2026-08-30): it is wired to `agentHub.getRecipes()` — the *sparkrun* catalog — so every `@dgxrun/` deploy (i.e. every multi-node deploy) misses the lookup and skips the check entirely. The dgxrun catalog already parses and validates `arch`; it is simply never consulted for admission. Newly live now that `qwen3.8-27b-nvfp4-rtx.yaml` puts an amd64 recipe in the same dropdown as ten arm64 ones.
  - [ ] **GPU class is still never compared.** `Node.gpuModel` exists and nothing checks it across cluster members, so a GB10 + 32 GiB RTX cluster passes on arch alone. No practical risk with four identical Sparks and one standalone RTX — left out of #44 deliberately.
...
- [ ] **Residual: the heartbeat wedge is reduced, not eliminated.** *(filed as [#36](https://github.com/kreuzhofer/dgx-manager/issues/36), 2026-08-30)* `inspectDgxrunContainer`, `snapshotDgxrunLogs` and `captureCrashedDgxrunLogs` are still `spawnSync`, so `[staleness] … marked offline` still fires during weight load — the deploy now *survives* it because `unknown ≠ absent`. Make the docker inspect async (`checkDgxrunDeployments` is already `async`). **Rule: never call `spawnSync` on the agent's hot path** — third incident of this class.
...
- ✅ **Accuracy-eval benchmark kind (lm-eval-harness)** — shipped 2026-07-09 (merged `2bb7730`). Third `kind` next to `throughput` + `tool-eval`: same `POST /api/benchmarks`, Benchmark button, detail card, list pill/score, and compare bars. **12 presets** (quick + full) across **IFEval, MMLU-Pro, GPQA-Diamond, GSM8K, BBH, MATH-hard** — the HF Open LLM Leaderboard v2 set minus MuSR. 903 tests green; server + dashboard `tsc` clean. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-09-accuracy-eval-lm-eval-benchmark*`.
  ...
  - [x] ~~Warm the `uvx` cache for lm-eval in `Dockerfile.server`~~ — **dropped 2026-08-30.** Both premises have expired: `Dockerfile.server` now warms llama-benchy **and** tool-eval-bench, and accuracy runs default to the eval node (`BENCH_RUNNER` defaults to `remote`, and a missing eval node 503s rather than falling back to the manager — `routes/benchmarks.ts:294-307`). The server image's uvx cache is therefore off the normal path; it still applies to opt-in `BENCH_RUNNER=local`, which is not worth a Dockerfile layer. Nothing warms uvx on the eval node either — that is where the original concern would now land if it is worth acting on at all.
...
- [ ] **Eval-runner comparability knobs — make apples-to-apples the default, not the exception.** The 2026-07-14 runs proved a benchmark number is a *measurement protocol*, not just a model — the gaps to official figures were mostly harness/scaffold/budget. To make our runs *comparable* rather than merely reproducible, add:
  - **Sampling config as preset fields** — `temperature`, `top_p`, `n` (samples), aggregation (greedy / maj@k / avg@k / pass@k). Our temp-0 single-sample systematically *undersells* vs frontier numbers that sample; matching the regime is a comparability fix, not a capability claim. (`buildLmEvalArgs` currently hardcodes greedy single-sample.)
  - **Comparison-target endpoint — the A/B, one run against two endpoints** (highest-leverage, fully in our control). Point the *same* harness invocation at the local INT4 endpoint AND a hosted full-precision GLM-5.2 (z.ai/provider API) → the only variable is precision → the diff is the genuine **quantization tax** we couldn't isolate this round. Sidesteps the vendor-harness problem entirely (both sides use *our* ruler).
  - **Per-preset "harness profile"** pinning a target's protocol — prompt template, answer extractor/parser (GPQA's `strict-match=0.0` was pure extraction mismatch, not weights), reasoning handling, and — for agentic kinds — the scaffold. A profile = "match z.ai's reported GPQA" vs "stock lm-eval" as an explicit, versioned choice.
  - **Pin dataset revision + metric per preset** (SWE-bench Verified≠Pro; GPQA flexible-extract vs strict; LiveCodeBench time-window) so every run states which number it's comparable to. Motivating writeup: [`docs/glm-5.2-benchmark-results.md`](glm-5.2-benchmark-results.md) + [[glm52-comparable-benchmarks-for-blog]].
- [ ] **Agent CPU% metric — filed as [#34](https://github.com/kreuzhofer/dgx-manager/issues/34) (2026-08-30).** Report per-node CPU utilization alongside the existing GPU/VRAM/thermal metrics (`agent metrics.ts` / `sysinfo`), and surface it in the dashboard. Especially useful now that agenthost is a CPU-bound eval/embedding node with no GPU metrics to show. *(requested 2026-07-11)*
  - Widened in the ticket to **load average and CPU temperature**, because two of the three are already collected and dropped: `load1/5/15` reach the manager on every tick and only `totalProcs` is read (`ws/agent-hub.ts:532`), and `tempC` is persisted (`agent-hub.ts:514,535`) but never rendered — the node card's "Temp" gauge is `nvidia-smi`'s GPU temp, not the CPU's. Only CPU% needs a genuinely new read (`/proc/stat` deltas).
  - **RAM vs VRAM is already done** and is not part of the ticket: `MemoryMetrics` → the "Memory" + "Swap" gauges on the node card.
...
- [ ] **`nodeIds:"auto"` sizing to exactly 1 node → spurious 400.** *(filed as [#35](https://github.com/kreuzhofer/dgx-manager/issues/35), 2026-08-30)* Pre-existing latent bug (unrelated to this branch): when auto-resolve needs exactly one node, `nodeIds.length===1` makes `isCluster` false, so `headNodeId` falls back to the never-populated legacy `nodeId` var and the request 400s with "nodeId or nodeIds required". Fix the solo/cluster branch in `routes/deployments.ts`.
...
- **🔥 GLM-5.2 broke overnight, then came back better — full story in `docs/glm-5.2-256k-to-320k.md`.**
  ...
  - **Two recipes now, one trade.** `@dgxrun/zai-glm-5.2-quanttrio-int4-unpruned-dcp2-256k-4x` stays the **SPEED** daily driver (256K, mnbt 2048, prefill ~698 tok/s); `…-dcp2-320k-4x` is the **CONTEXT** sibling (320K, mnbt 1024, prefill 619.5 tok/s). Decode identical. Everything else — DCP2, tp=4, cudagraph FULL, MTP drafter — is the same in both.
...
- **Ollama governance** — startup firewall restricts `:11434` to manager + loopback; install without autostart on **both** provisioning paths (an Ollama that starts at boot is briefly unfiltered, since the agent applies the firewall when it starts); on-demand service start for Ollama deploys, and persisted Ollama deployments reconciled on agent reconnect so a reboot does not strand them; firewall state reported via self-audit
