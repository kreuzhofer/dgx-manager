...
- [ ] **SWE-bench inference scaffold.** Single-shot oracle patches from GLM-5.2 often fail `git apply` (e.g. a *correct* astropy-12907 fix carried a redundant no-op hunk). Enforce patch format / retry on apply-fail / consider an agentic loop before the SWE-bench number is meaningful.
- [ ] **Comparable-benchmark blog runs (blocked/queued):** GPQA-Diamond full (blocked on the HF_TOKEN above — the ONE apples-to-apples number vs official 91.2), then AIME 2026 + LiveCodeBench (need harness setup; run sequentially — 409 guard + single GLM-5.2 endpoint). See the [[glm52-comparable-benchmarks-for-blog]] memory.
