...
- **Next lever = CONTEXT via DCP (decode-context-parallel), for agentic-coding daily-driver use** (opencode / Claude Code need big windows). DCP shards MLA KV across ranks (context × N; decode unaffected ~22, prefill ~linear cost). Cheapest path may be the **pruned model at ~327K WITHOUT DCP** (no rebuild — community's no-DCP row reaches 327K on the 10% prune); unpruned at 320K/640K needs the **DCP-stack rebuild** (local-inference-lab/vllm `codex/dcp-*` branch + PR#72 + `VLLM_USE_V2_MODEL_RUNNER=1`) — a *context* project, not a speed one. Pruned-vs-unpruned decided by the quality baseline (IFEval + tool-eval, in progress).
